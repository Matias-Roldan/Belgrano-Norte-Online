...
// [SEC-FIX] SVG por tipo — sin emojis, sin atributos duplicados que rompan el parser
function IconoNivel({ tipo, size, color }) {
  const sz = size || 16;
  const c  = color || '#555';

  if (tipo === 'crit') {
    return (
      <svg width={sz} height={sz} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
  ...
  // info — [SEC-FIX] removido strokeWidth inline que rompía el parser JSX
  return (
    <svg width={sz} height={sz} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
      <circle cx="12" cy="12" r="10"/>
...
function sanitizarTexto(str, max) {
  var m = max || 500;
  if (typeof str !== 'string') return '';
  return str.replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/g, '').trim().slice(0, m);
}

function validarAviso(a) {
  if (!a || typeof a !== 'object') return null;
  var id    = parseInt(a.id_aviso, 10);
  var nivel = parseInt(a.nivel, 10);
...
var formatFecha = function(fechaISO) {
  if (!fechaISO) return '';
...
function TarjetaAviso({ aviso }) {
  var cfg   = NIVEL[aviso.nivel] || NIVEL[1];
  var fecha = formatFecha(aviso.fecha_creacion);
  ...
  return (
    <div style={s.root}>
      ...
      <main style={s.main}>
        ...
        <div role="status" aria-live="polite" style={{ ...s.estadoBanner, background: estado.bg, borderColor: estado.borde }}>
